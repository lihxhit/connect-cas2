  ...
  if (ticket) {
    ...
    validateTicket(req, options, function(err, response) {
      ...
      if (response.status === 200) {
        ...
        parseCasResponse(response.body, logger,options, function(err, info) {
          if (err) {
            var resBody = {
              error: err
            };
          ...
          if (!pgtIou) {
            if (options.paths.proxyCallback) {
              ...
              let onSuccessLogin = options.onSuccessLogin;
              if(!onSuccessLogin){
                onSuccessLogin = function(req,res,successCallback){
                  successCallback();
                }
              }
              onSuccessLogin(req,res,function(){
 ...
 * @param callback
 */
function parseCasResponse(casBody, logger,options, callback) {
  xml2js(casBody, {
    explicitRoot: false,
    ...
    if(options.explicitArray){
      success = serviceResponse.authenticationSuccess && serviceResponse.authenticationSuccess[0];
    }else{
